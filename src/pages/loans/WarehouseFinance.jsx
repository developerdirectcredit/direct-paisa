...
//              Fuel your warehousing and logistics growth with tailored financing solutions for new warehouse construction, capacity expansion, modernization, and infrastructure development.
//               Avail funding from ₹50 Lakhs to ₹250 Crores, flexible repayment options, and Government subsidy benefits (where applicable) with end-to-end expert assistance.
...
/*  Warehouse Receipt Finance (Lease Rental)                          */
/*  Hero = lead capture (Name + Mobile) + 35% subsidy highlight,       */
/*         restyled to Agri Loan's navy (#001f54) / red (#e8112d)      */
/*         theme, with new Repayment Stats / Representative-cost-      */
/*         table content below it.                                    */
/*  Wizard: 1 Warehouse → 2 Loan → 3 Loan Amount → 4 Tenant →          */
/*          5 Documents → 6 Review → bank offers → toast. (unchanged)  */
/* ------------------------------------------------------------------ */
...
const repaymentStats = [
  { icon: Calendar, title: "Minimum Repayment Period", desc: "05 months" },
...
const loanCostDetails = [
  { feature: "Facility Type", details: "Warehouse Loan / Agri Infrastructure Loan" },
  { feature: "Loan Amount", details: "₹1,00,00,000" },
  { feature: "Purpose", details: "Construction / Modernization of warehouse for storage of agriculture commodities / supply chain services / e-marketing platforms" },
  { feature: "Tenure", details: "120 months (10 years)" },
  { feature: "Moratorium Period", details: "Up to 18 months (included in tenure)" },
  { feature: "Interest Rate (Indicative)", details: "9.50% p.a." },
  { feature: "Monthly Installment (EMI)", details: "₹1,29,400 (approx.)" },
  { feature: "Total Amount Payable", details: "₹1,56,00,000 (approx.)" },
  { feature: "Total Interest Cost", details: "₹56,00,000 (approx.)" },
  { feature: "Processing / Bank Fee", details: "0.25% – 1% of loan amount" },
  { feature: "Collateral Requirement", details: "Warehouse property + additional security, as applicable" },
  { feature: "Repayment", details: "Monthly EMI after moratorium period" },
  { feature: "Subsidy", details: "3% p.a. interest subsidy from govt. (in applicable cases only)" },
];
        ...
        <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
          ...
          <div>
            ...
            <h1 className="text-4xl font-extrabold leading-tight tracking-tight lg:text-5xl">
              ...
              <span className="text-[#001f54]">Finance</span>
            </h1>
            <p className="mt-4 max-w-xl text-slate-600">
             Fuel your warehousing and logistics growth with tailored financing solutions for new warehouse construction, capacity expansion, modernization, and infrastructure development.
              Avail funding from ₹50 Lakhs to ₹250 Crores, flexible repayment options, and Government subsidy benefits (where applicable) with end-to-end expert assistance.
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:p-8">
            ...
            <p className="mt-1 text-center text-2xl font-bold">
              Fund against your{" "}
              <span className="text-[#e8112d]">warehouse</span>
            </p>
            ...
            <div className="mt-6 space-y-4">
              ...
              <label className="flex items-start gap-2 text-xs text-slate-500">
                <input type="checkbox" checked={agree} onChange={(e) => { setAgree(e.target.checked); setErr((p) => ({ ...p, agree: undefined })); }} className="mt-0.5 accent-blue-600" />
                <span>By submitting this form, you agree to the Credit Report Terms of Use, Terms of Use &amp; Privacy Policy.</span>
              </label>
              {err.agree && <ErrorText>{err.agree}</ErrorText>}
            </div>
...
function Highlight({ label, value }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-3 py-3 text-center">
      <p className="text-base font-bold text-[#001f54]">{value}</p>
      <p className="text-xs text-slate-500">{label}</p>
    </div>
  );
}
    ...
    <div className="flex items-center justify-between overflow-x-auto">
      {STEP_LABELS.map((label, i) => {
        ...
        return (
          <div key={label} className="flex flex-1 items-center">
            <div className="flex flex-col items-center">
              <div className={`grid h-8 w-8 place-items-center rounded-full text-xs font-semibold ${done ? "bg-[#001f54] text-white" : active ? "bg-blue-600 text-white ring-4 ring-blue-100" : "bg-slate-200 text-slate-500"}`}>
                {done ? <Check className="h-4 w-4" /> : n}
              </div>
              <span className="mt-2 hidden whitespace-nowrap text-[11px] text-slate-500 sm:block">{label}</span>
            </div>
            {n < STEP_LABELS.length && <div className={`mx-1 h-0.5 flex-1 ${n < step ? "bg-[#001f54]" : "bg-slate-200"}`} />}
          </div>
        );
      })}
    </div>
